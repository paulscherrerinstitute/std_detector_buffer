...
import struct
from std_buffer.eiger.data import EG_MAX_PAYLOAD, EgUdpPacket, eg_udp_packet_to_frame, calculate_udp_packet_info


def calculate_udp_packet_info_original(image_height, image_width, bit_depth, link_id=0):
    ...
    udp_blocks_48p = int((EG_MAX_PAYLOAD / (36 * count_x_12ppq)) * count_x_12ppq / 2)
...
if __name__ == '__main__':
    for y in range(4, 514, 4):
        for x in range(48, 1030, 12):
    ...
    image_pixel_width = 514
    image_pixel_height = 1030
    n_images = 2
    bit_depth = 16
    ...
    for i in range(frame_info['frame_n_packets'] * n_images):
        ...
        for input_file in files:
            ...
            packet = EgUdpPacket.from_buffer_copy(data)
            frame = eg_udp_packet_to_frame(packet, module_n_x_pixel, module_n_y_pixel, frame_info['frame_n_packets'], bit_depth)
