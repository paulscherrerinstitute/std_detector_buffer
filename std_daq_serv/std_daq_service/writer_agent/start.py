...
def main():
    ...
    parser.add_argument(
        "detector_name", type=str, help="Name of the detector to write."
    )
    ...
    _logger.info(
        f"Service {args.service_name} connecting to {args.broker_url}."
    )
    ...
    _logger.info(
        f"Receiving from {input_stream} and sending data to {output_stream}."
    )

    service = RequestWriterService(
        input_stream_url=input_stream, output_stream_url=output_stream
    )

    listener = PrimaryBrokerService(
