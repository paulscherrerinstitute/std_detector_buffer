...
ucx_rdma_endpoint::ucx_rdma_endpoint(std::span<std::byte> buffer)
    : buffer_(buffer)
    , is_server_(true)
{
  // 1. UCX context
...
// --- Client-side constructor: does not register memory or pack rkey ---
ucx_rdma_endpoint::ucx_rdma_endpoint()
    : is_server_(false)
{
  ucp_params_t params{};
  params.field_mask = UCP_PARAM_FIELD_FEATURES;
...
// --- For server demo: put data in buffer ---
...
// --- For client: create endpoint to remote worker ---
...
// --- For client: unpack rkey for remote memory ---
