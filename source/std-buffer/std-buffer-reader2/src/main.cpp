...
void do_accept(boost::asio::ip::tcp::acceptor& acceptor,
               ...
               std::shared_ptr<sbr::writer_driver> driver)
{
  ...
  acceptor.async_accept(*socket, [&, socket, manager, driver](boost::system::error_code ec) {
    if (!ec)
      std::make_shared<sbr::socket_session>(std::move(*socket), manager, driver)->start();
...
int main(int argc, char* argv[])
{
  ...
  auto driver =
      std::make_shared<sbr::writer_driver>(sm, source_suffix, config, with_metadata_writer);
