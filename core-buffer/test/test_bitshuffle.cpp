...
TEST(Bitshuffle, SimpleCompression)
{
  auto compress_buffer_size =
      bshuf_compress_lz4_bound(MODULE_N_PIXELS, PIXEL_N_BYTES, MODULE_N_PIXELS);
  ...
  auto compressed_size = bshuf_compress_lz4(frame_buffer.get(), compress_buffer.get(),
  ...
  auto consumed_bytes = bshuf_decompress_lz4(compress_buffer.get(), out_frame_buffer.get(),
                                             MODULE_N_PIXELS, PIXEL_N_BYTES, MODULE_N_PIXELS);
...
TEST(Bitshuffle, SeparateCompression)
{
  ...
  auto compressed_size_1 = bshuf_compress_lz4(frame_buffer.get(), compress_buffer_1.get(),
                                              MODULE_N_PIXELS, PIXEL_N_BYTES, MODULE_N_PIXELS);
  ...
  auto compressed_size_2 = bshuf_compress_lz4(frame_buffer.get(), compress_buffer_2.get(),
                                              MODULE_N_PIXELS, PIXEL_N_BYTES, MODULE_N_PIXELS);
  ...
  memcpy(&(sum_compressed_buffer[0]), &(compress_buffer_1[0]), compressed_size_1);
  ...
  auto consumed_bytes = bshuf_decompress_lz4(sum_compressed_buffer.get(), out_frame_buffer.get(),
...
TEST(Bitshuffle, CompressionSpeed)
{
  ...
  auto compression_buffer =
      make_unique<char[]>(bshuf_compress_lz4_bound(n_pixels, PIXEL_N_BYTES, 0));
  ...
  size_t duration = duration_cast<microseconds>(end_time - start_time).count();
