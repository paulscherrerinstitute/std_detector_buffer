...
void ZmqLiveSender::send(const ImageMetadata& meta, const char* data, const size_t image_n_bytes)
{
  ...
  // TODO: Here we need to send to streamvis and live analysis metadata(probably need to operate
  // still on them) and data(not every frame)
  ...
  zmq_send(socket_streamvis_, text_header.c_str(), text_header.size(), ZMQ_SNDMORE);

  if (send_streamvis == 0) {
    zmq_send(socket_streamvis_, (char*)data, image_n_bytes, 0);
  }
  else {
    zmq_send(socket_streamvis_, (char*)data_empty, 8, 0);
  }
}
