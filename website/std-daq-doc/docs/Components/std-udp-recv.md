...
[std-udp-recv](https://github.com/paulscherrerinstitute/sf_daq_buffer/tree/eiger/std-udp-recv) receives the detector data in form of UDP packages and writes it down to disk or ram. In addition, it sends a copy of the module frame to std-stream-send via ZMQ.
...
- **FORMAT_MARKER** (0xBE) - a control byte to determine the validity of the frame.
...
- **detector_folder** should always be passed as an absolute path. This is the
  container that holds all data related to a specific detector.
- **module_folder** is usually composed like "M00", "M01". It separates data
...
will therefore be **FILE_MOD / FOLDER_MOD = 100**.
