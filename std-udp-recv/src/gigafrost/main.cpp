...
int main(int argc, char* argv[])
{
  ...
  const auto detector_config = buffer_utils::read_json_config(std::string(argv[1]));
  ...
  PacketUdpReceiver receiver(detector_config.start_udp_port + module_id, sizeof(GFUdpPacket),
                             FRAME_N_PACKETS);
  FrameStats stats(detector_config.detector_name, module_id, buffer_config::STATS_TIME);
