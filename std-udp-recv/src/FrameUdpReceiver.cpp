...
template <typename T> FrameUdpReceiver<T>::~FrameUdpReceiver()
{
...
void FrameUdpReceiver<T>::copy_packet_to_buffers(char* frame_buffer, const int i_packet)
{
  ...
  memcpy((void*)(frame_buffer + frame_buffer_offset), packet_buffer_[i_packet].data,
...
uint64_t FrameUdpReceiver<T>::process_packets(const int start_offset, T& meta, char* frame_buffer)
{
  for (int i_packet = start_offset; i_packet < packet_buffer_n_packets_; i_packet++) {
    ...
    if (packet_buffer_[i_packet].packetnum == n_packets_per_frame_ - 1) {
#ifdef DEBUG_OUTPUT
      using namespace date;
      cout << " [" << std::chrono::system_clock::now();
...
template <typename T> uint64_t FrameUdpReceiver<T>::get_frame_from_udp(T& meta, char* frame_buffer)
{
